...
"""
    ...
    def _buscar_lugar_mejorado(self, lat: float, lng: float, direccion: str,
                               nombre_cuenta: str, nombre_obra: str, municipio: str):
        """
        ...
        if tipo_cuenta == "empresa" and nombre_cuenta and nombre_cuenta.strip():
            ...
            for intento in intentos:
                ...
                lugar = self.buscador.buscar_lugar(lat, lng, intento)
                if lugar:
            ...
            # Si no encontro por nombre, NO hacer fallback - evita falsos positivos
            print(f"   1B EMPRESA: No encontre por nombre exacto.")

        # ESTRATEGIA B: Si es PERSONA, buscar por NOMBRE_OBRA
        if tipo_cuenta == "persona":
            if nombre_obra and nombre_obra.strip():
                print(f"   2 PERSONA: Buscando por NOMBRE_OBRA: {nombre_obra}")
                lugar = self.buscador.buscar_lugar(lat, lng, nombre_obra)
